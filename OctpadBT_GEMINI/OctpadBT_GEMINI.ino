//////// code
// https://g.co/gemini/share/651cfb4a6e43
...
// The code has been updated to only use ADC1 pins for stable operation.

const int NUM_PADS = 6;
const int piezoPins[NUM_PADS] = {32, 33, 34, 35, 36, 39};

// Mapping of GPIO pins to ADC1 channels
const adc1_channel_t adc1_channels[NUM_PADS] = {
...
void updateConfigFile(int padNumber, const char* waveFileName) {
  if (padNumber < 0 || padNumber >= NUM_PADS) {
    Serial.println("Invalid pad number. Please use a number between 0 and 5.");
...
void parseBluetoothCommand(String command) {
  ...
  if (commaIndex == -1) {
    Serial.println("Invalid command format. Use 'pad,filename.wav'");
    SerialBT.println("ERROR: Invalid command format. Use 'pad,filename.wav'");
    return;
  }
  
  String padString = command.substring(0, commaIndex);
  String waveFileName = command.substring(commaIndex + 1);
  
  int padNumber = padString.toInt();
  
  // Call the update function with the parsed values
...
void setup() {
 ...
 SerialBT.begin("ESP32-Drum-Machine");
